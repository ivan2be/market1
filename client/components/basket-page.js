...
const Basket = () => {
  ...
  return Object.keys(cart).length === 0 ? (
    <div className="flex justify-center text-center text-4xl font-bold py-64">
    ...
    <div className="flex flex-col px-48">
      <table className="table-auto">
        <thead>
          <tr className="text-2xl font-bold text-center">
            <th className="w-1/10 px-1 py-2">Image </th>
            <th className="w-6/10 px-1 py-2">Title</th>
            <th className="w-1/10 px-1 py-2">Price</th>
            <th className="w-1/10 px-1 py-2">Total price</th>
            <th className="w-1/10 px-1 py-2">Amount</th>
          </tr>
        </thead>
        <tbody>
      ...
      <div className="flex justify-end text-2xl font-bold px-12">
        Total: {getTotalAmount(cart, items, currency, exchangeRate)}
      </div>
    </div>
  )
}
